...
TestNode::TestNode(uint8_t command)
{
    switch(command){
        ...
        case CMD_GET_HIGH_STATE:
            ...
            high_state_client = node->create_client<a1_msgs::srv::HighState>(ROS2_SERVICE_GET_HIGH_STATE_MSG);
        ...
        case CMD_GET_CARTESIAN:
            ...
            cartesian_client = node->create_client<a1_msgs::srv::Cartesian>(ROS2_SERVICE_GET_CARTESIAN_MSG);
        default:
            return;
    }
}
...
void TestNode::pub_pose(float yaw, float pitch, float roll, float bodyHeight) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "yaw[%f], pitch[%f], roll[%f], bodyHeight[%f]",
...
bool TestNode::pub_velocity(float forwardSpeed, float sideSpeed, float rotateSpeed) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "sending forwardSpeed[%f],sideSpeed[%f],rotateSpeed[%f]",
